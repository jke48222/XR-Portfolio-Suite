    ...
    public class EarthElementController : MonoBehaviour, IElementController
    {
        ...
        public void ProcessIntent(in GestureIntentData intent, float deltaTime)
        {
            ...
            if (_currentState == ElementState.Sustaining || _currentState == ElementState.Engaging)
            {
                ...
                _rb.AddForce(-_rb.linearVelocity * _dynamics.InstabilityDamping * instability, ForceMode.Acceleration);
                _rb.linearVelocity = _dynamics.ClampVelocity(_rb.linearVelocity * Mathf.Lerp(1f, 0.9f, instability));
            }
            else if (_currentState == ElementState.Releasing)
            {
                _rb.AddForce(-_rb.linearVelocity * (_dynamics.InstabilityDamping + 0.5f) * deltaTime, ForceMode.VelocityChange);
            }
